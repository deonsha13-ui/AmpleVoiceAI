...
function Download-BundleFileForRef([string]$Destination, [string]$RemotePath, [string]$Ref) {
    $rawBase = "https://raw.githubusercontent.com/amplevoice-hq/amplevoice/$Ref"
    $fallbackBase = 'https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main'
...
function Download-InitSupportBundle([string]$ProjectDir, [string]$Ref) {
    ...
    Download-BundleFileForRef (Join-Path $ProjectDir 'scripts/run_amplevoice_init.sh') 'scripts/run_amplevoice_init.sh' $Ref
...
Write-Info ''
Write-Info '╔══════════════════════════════════════════════════════════════╗'
Write-Info '║                    AmpleVoiceAI Local Setup                        ║'
...
$Registry = if ([string]::IsNullOrEmpty($env:REGISTRY)) { 'ghcr.io/amplevoice-hq' } else { $env:REGISTRY }
...
if ($env:AMPLEVOICE_SKIP_DOWNLOAD -ne '1') {
    if ($UseCoturn) {
    ...
    Download-File 'https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/docker-compose.yaml' (Join-Path $CurrentDir 'docker-compose.yaml')
    if ($UseCoturn) {
...
if ($UseCoturn) {
    Assert-PathExists 'scripts/run_amplevoice_init.sh' 'scripts/run_amplevoice_init.sh not found. Re-run setup_local.ps1 without AMPLEVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'scripts/lib/setup_common.sh' 'scripts/lib/setup_common.sh not found. Re-run setup_local.ps1 without AMPLEVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
    Assert-PathExists 'deploy/templates/turnserver.remote.conf.template' 'deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.ps1 without AMPLEVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout.'
}
...
$envLines = @(
    '# Container registry for AmpleVoiceAI images'
    "REGISTRY=$Registry"
    ''
