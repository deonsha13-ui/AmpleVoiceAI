...
AMPLEVOICE_DIR="$(dirname "$SCRIPT_DIR")"
...
# Install amplevoice API requirements first so pipecat's extras win on any
# shared transitive dependencies (matches api/Dockerfile and CI workflow).
echo "Installing amplevoice API requirements..."
uv pip install -r api/requirements.txt

if [ "$DEV_MODE" -eq 1 ]; then
    echo "Installing amplevoice API dev requirements..."
