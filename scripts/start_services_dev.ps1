#!/usr/bin/env pwsh
# Start AmpleVoiceAI services in development mode (Windows)
...
$EnvFile    = if ($env:AMPLEVOICE_ENV_FILE) { $env:AMPLEVOICE_ENV_FILE } else { Join-Path $BaseDir 'api/.env' }
...
Write-Host "Starting AmpleVoiceAI Services (DEV MODE) in BASE_DIR: $BaseDir"
