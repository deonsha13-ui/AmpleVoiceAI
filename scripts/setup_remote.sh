...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║                   AmpleVoiceAI Remote Setup                        ║"
...
if [[ -z "$SERVER_IP" ]]; then
    amplevoice_fail "IP address cannot be empty"
fi

if ! amplevoice_is_ipv4 "$SERVER_IP"; then
    amplevoice_fail "Invalid IP address format"
fi
...
if [[ -z "${DEPLOY_MODE:-}" ]]; then
    if [[ -t 0 ]]; then
        ...
        echo "  1) prebuilt - pull official amplevoice images (recommended, fastest)"
        ...
        case "$mode_choice" in
            ...
            *) amplevoice_fail "invalid choice '$mode_choice'" ;;
        esac
    else
    ...
    if [[ "$REPO_SOURCE" == "clone" ]]; then
        if [[ -z "${FORK_REPO:-}" ]]; then
            if [[ -t 0 ]]; then
                ...
                read -p "[amplevoice-hq/amplevoice]: " FORK_REPO
                FORK_REPO="${FORK_REPO:-amplevoice-hq/amplevoice}"
            else
                FORK_REPO="amplevoice-hq/amplevoice"
            fi
        fi
...
if [[ "$DEPLOY_MODE" == "build" && "${REPO_SOURCE:-}" == "existing" ]]; then
    ...
    TARGET_DIR="amplevoice"
fi

if [[ "${AMPLEVOICE_FORCE_OVERWRITE:-}" != "1" && "${AMPLEVOICE_SKIP_DOWNLOAD:-}" != "1" ]]; then
    if [[ -f "$TARGET_DIR/.env" ]]; then
        ...
        echo ""
        echo -e "${YELLOW}Detected an existing AmpleVoiceAI install:${NC}"
        ...
        echo -e "  ${BLUE}https://docs.amplevoice.com/deployment/update${NC}"
        echo ""
        echo -e "${BLUE}To wipe state and reinstall from scratch, re-run with:${NC}"
        echo -e "  ${BLUE}AMPLEVOICE_FORCE_OVERWRITE=1 <same command>${NC}"
        echo ""
        exit 1
    fi
fi
...
if [[ "$DEPLOY_MODE" == "build" ]]; then
    if [[ "${AMPLEVOICE_SKIP_DOWNLOAD:-}" == "1" ]]; then
        echo -e "${BLUE}[1/$TOTAL] Using existing repo in current directory${NC}"
    elif [[ "${REPO_SOURCE:-}" == "clone" ]]; then
        if [[ -e "amplevoice" ]]; then
            amplevoice_fail "'amplevoice' directory already exists. Remove it or re-run with REPO_SOURCE=existing from inside it."
        fi
        echo -e "${BLUE}[1/$TOTAL] Cloning $FORK_REPO (branch: $BRANCH)...${NC}"
        git clone --branch "$BRANCH" --recurse-submodules "https://github.com/$FORK_REPO.git" amplevoice
        cd amplevoice
    ...
    if [[ "${AMPLEVOICE_SKIP_DOWNLOAD:-}" != "1" ]]; then
        mkdir -p amplevoice 2>/dev/null || true
        cd amplevoice

        echo -e "${BLUE}[1/$TOTAL] Downloading deployment bundle...${NC}"
        curl -fsSL -o docker-compose.yaml "https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/docker-compose.yaml"
...
services:
  api:
    ...
    image: amplevoice-local/amplevoice-api:local
    pull_policy: never
  ...
  ui:
    ...
    image: amplevoice-local/amplevoice-ui:local
    pull_policy: never
OVERRIDE_EOF
...
echo "  - .env"
echo ""
echo -e "${YELLOW}To start AmpleVoiceAI, run:${NC}"
echo ""
if [[ "$DEPLOY_MODE" != "build" || "${REPO_SOURCE:-}" != "existing" ]]; then
