...
# Requires:
#   - `python` in the `amplevoice` conda env, `api/.env` sourced; the `api`
...
SPECS_JSON="$(mktemp -t amplevoice-specs-XXXXXX.json)"
OPENAPI_JSON="$(mktemp -t amplevoice-openapi-XXXXXX.json)"
...
PYTHONPATH="$REPO_ROOT/sdk/python/src" python -m amplevoice_sdk.codegen \
    --input "$SPECS_JSON" \
...
datamodel-codegen \
    ...
    --output "sdk/python/src/amplevoice_sdk/_generated_models.py" \
...
python -m sdk.codegen.client_codegen \
    --input "$OPENAPI_JSON" \
    --py-out "sdk/python/src/amplevoice_sdk/_generated_client.py" \
