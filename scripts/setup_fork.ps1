...
$UpstreamUrl    = 'https://github.com/amplevoice-hq/amplevoice.git'
$CanonicalHttps = $UpstreamUrl
$CanonicalSsh   = 'git@github.com:amplevoice-hq/amplevoice.git'
...
Write-Host '|              AmpleVoiceAI Contributor Bootstrap                    |' -ForegroundColor Blue
Write-Host '+==============================================================+' -ForegroundColor Blue
Write-Host ''
...
if ($needsForkPrompt) {
    Write-Host 'Enter your fork URL (e.g. https://github.com/<YOUR_HANDLE>/amplevoice.git):' -ForegroundColor Yellow
...
if (-not $existingUpstream) {
    ...
    $reset = (Read-Host 'Reset upstream to amplevoice-hq/amplevoice? [y/N]').Trim()
    if ($reset -match '^[Yy]') {
