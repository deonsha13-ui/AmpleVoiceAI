...
REGISTRY="${REGISTRY:-ghcr.io/amplevoice-hq}"
...
# Download compose file (skip when AMPLEVOICE_SKIP_DOWNLOAD=1 — e.g. local repo testing).
TOTAL_STEPS=2
...
if [[ "${AMPLEVOICE_SKIP_DOWNLOAD:-}" != "1" ]]; then
    ...
    curl -sS -o docker-compose.yaml https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/docker-compose.yaml
    if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
        amplevoice_download_init_support_bundle "$(pwd)" "main"
    fi
...
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    [[ -f scripts/run_amplevoice_init.sh ]] || amplevoice_fail "scripts/run_amplevoice_init.sh not found. Re-run setup_local.sh without AMPLEVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f scripts/lib/setup_common.sh ]] || amplevoice_fail "scripts/lib/setup_common.sh not found. Re-run setup_local.sh without AMPLEVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout."
    [[ -f deploy/templates/turnserver.remote.conf.template ]] || amplevoice_fail "deploy/templates/turnserver.remote.conf.template not found. Re-run setup_local.sh without AMPLEVOICE_SKIP_DOWNLOAD=1, or use a full repo checkout."
fi
...
echo ""
if [[ "${ENABLE_COTURN:-false}" == "true" ]]; then
    echo -e "${YELLOW}To start AmpleVoiceAI with TURN, run:${NC}"
    echo ""
