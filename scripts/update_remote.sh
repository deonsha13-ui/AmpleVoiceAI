...
REPO="amplevoice-hq/amplevoice"
TIMESTAMP=$(date +%Y%m%d-%H%M%S)

echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║                  AmpleVoiceAI Remote Update                        ║"
echo "║  Refresh deployment files and validate runtime config        ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

[[ -f docker-compose.yaml ]] || amplevoice_fail "docker-compose.yaml not found in $(pwd)"
[[ -f .env ]] || amplevoice_fail ".env not found in $(pwd)"
...
if [[ -f docker-compose.override.yaml ]]; then
    ...
    echo ""
    echo -e "${YELLOW}See https://docs.amplevoice.com/deployment/update#updating-a-source-build${NC}"
    exit 1
fi
...
[[ -n "${TURN_SECRET:-}" ]] || amplevoice_fail "TURN_SECRET not found in .env"
...
[[ "$FASTAPI_WORKERS" =~ ^[1-9][0-9]*$ ]] || amplevoice_fail "FASTAPI_WORKERS must be a positive integer (got: $FASTAPI_WORKERS)"

TARGET_VERSION="${_caller_TARGET_VERSION:-${TARGET_VERSION:-}}"

if [[ -z "$TARGET_VERSION" ]]; then
    amplevoice_info "Fetching latest release tag from GitHub..."
    LATEST_TAG=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)

    if [[ -z "$LATEST_TAG" ]]; then
        amplevoice_warn "Could not auto-discover latest tag - defaulting to 'main'."
        LATEST_TAG="main"
    fi
    ...
    if [[ -t 0 ]]; then
        ...
        echo -e "${YELLOW}full git tag (amplevoice-v1.28.0), or 'main' for bleeding edge.${NC}"
...
if [[ "$TARGET_VERSION" == "latest" ]]; then
    TARGET_VERSION=$(curl -fsSL "https://api.github.com/repos/$REPO/releases/latest" 2>/dev/null \
        | grep -E '"tag_name":' | head -1 \
        | sed -E 's/.*"tag_name":[[:space:]]*"([^"]+)".*/\1/' || true)
    [[ -n "$TARGET_VERSION" ]] || amplevoice_fail "could not resolve 'latest' to a release tag"
fi
...
case "$TARGET_VERSION" in
    ...
    amplevoice-*)
        ;;
    v*)
        TRY_TAGS+=("amplevoice-$TARGET_VERSION")
        ;;
    *)
        TRY_TAGS+=("amplevoice-v$TARGET_VERSION" "v$TARGET_VERSION" "amplevoice-$TARGET_VERSION")
        ;;
esac

amplevoice_info "Validating target version: $TARGET_VERSION..."
RESOLVED_TAG=""
...
[[ -n "$RESOLVED_TAG" ]] || amplevoice_fail "could not find a git tag matching '$TARGET_VERSION'"

if [[ "$RESOLVED_TAG" != "$TARGET_VERSION" ]]; then
    amplevoice_success "✓ Resolved '$TARGET_VERSION' to git tag '$RESOLVED_TAG'"
fi
...
case "$TARGET_VERSION" in
    amplevoice-v*) IMAGE_TAG="${TARGET_VERSION#amplevoice-v}" ;;
...
if [[ -n "$IMAGE_TAG" ]]; then
    if curl -fsI "https://hub.docker.com/v2/repositories/amplevoiceai/amplevoice-api/tags/$IMAGE_TAG/" >/dev/null 2>&1; then
        amplevoice_success "✓ Image tag :$IMAGE_TAG found on Docker Hub"
    else
        amplevoice_warn "Warning: image tag :$IMAGE_TAG not found on Docker Hub - leaving images at :latest"
        IMAGE_TAG=""
    fi
fi
...
echo -e "  Server IP:        ${BLUE}$(amplevoice_infer_server_ip "$(pwd)" || echo "unknown")${NC}"
...
echo "  - scripts/run_amplevoice_init.sh"
...
if [[ -t 0 && "${AMPLEVOICE_UPDATE_YES:-}" != "1" ]]; then
...
for f in \
    ...
    .env \
    remote_up.sh \
    scripts/run_amplevoice_init.sh \
...
amplevoice_download_remote_support_bundle "$(pwd)" "$TARGET_VERSION"
rm -f nginx.conf turnserver.conf

if [[ -n "$IMAGE_TAG" ]]; then
    sed -i.tmp -E "s#(amplevoice-(api|ui)):latest#\1:$IMAGE_TAG#g" docker-compose.yaml
    rm -f docker-compose.yaml.tmp
    amplevoice_success "✓ docker-compose.yaml updated; images pinned to :$IMAGE_TAG"
else
    amplevoice_success "✓ docker-compose.yaml updated (image tags left at :latest)"
fi

echo -e "${BLUE}[3/3] Synchronizing environment and validating init-based remote config...${NC}"
amplevoice_set_env_key .env FASTAPI_WORKERS "$FASTAPI_WORKERS"
amplevoice_prepare_remote_install "$(pwd)"
docker compose config -q
amplevoice_success "✓ Remote init configuration validated"
...
echo ""
echo -e "  ${BLUE}for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_amplevoice_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do${NC}"
