...
if [[ ! -f "$LIB_PATH" ]]; then
    ...
    curl -fsSL -o "$BOOTSTRAP_LIB" "https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/lib/setup_common.sh"
...
echo -e "${BLUE}"
echo "╔══════════════════════════════════════════════════════════════╗"
echo "║              AmpleVoiceAI Custom Domain Setup                      ║"
echo "║     Automated Let's Encrypt SSL certificate setup            ║"
echo "╚══════════════════════════════════════════════════════════════╝"
echo -e "${NC}"

if [[ $EUID -ne 0 ]]; then
    amplevoice_fail "This script must be run as root or with sudo"
fi

if [[ ! -d "amplevoice" ]]; then
    echo -e "${RED}Error: 'amplevoice' directory not found.${NC}"
    echo -e "${YELLOW}Please run this script from the directory containing your AmpleVoiceAI installation.${NC}"
    echo -e "${YELLOW}If you haven't set up AmpleVoiceAI yet, run the remote setup first:${NC}"
    echo -e "${BLUE}  curl -o setup_remote.sh https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh${NC}"
    exit 1
fi

echo -e "${YELLOW}Enter your domain name (e.g., voice.yourcompany.com):${NC}"
read -p "> " DOMAIN_NAME
[[ -n "$DOMAIN_NAME" ]] || amplevoice_fail "Domain name cannot be empty"

if ! [[ "$DOMAIN_NAME" =~ ^[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?(\.[a-zA-Z0-9]([a-zA-Z0-9-]*[a-zA-Z0-9])?)*\.[a-zA-Z]{2,}$ ]]; then
    amplevoice_fail "Invalid domain name format"
fi

echo -e "${YELLOW}Enter your email address for SSL certificate notifications:${NC}"
read -p "> " EMAIL_ADDRESS
[[ -n "$EMAIL_ADDRESS" ]] || amplevoice_fail "Email address cannot be empty (required by Let's Encrypt)"
...
if [[ -z "$SERVER_IP" ]]; then
    amplevoice_warn "Warning: Could not detect server's public IP"
...
if command -v apt-get &> /dev/null; then
    ...
    amplevoice_fail "Could not detect package manager. Please install certbot manually."
fi
echo -e "${GREEN}✓ Certbot installed${NC}"

echo -e "${BLUE}[3/7] Stopping AmpleVoiceAI services...${NC}"
cd amplevoice
AMPLEVOICE_DEPLOY_PROJECT_DIR="$(pwd)"

if [[ ! -f remote_up.sh || ! -f scripts/lib/setup_common.sh ]]; then
    amplevoice_download_remote_support_bundle "$(pwd)" "main"
fi

amplevoice_require_init_compose_layout "$(pwd)"

if docker compose --profile remote ps --quiet 2>/dev/null | grep -q .; then
    docker compose --profile remote down
    echo -e "${GREEN}✓ AmpleVoiceAI services stopped${NC}"
else
...
amplevoice_load_env_file .env

if [[ -z "${SERVER_IP:-}" ]]; then
    SERVER_IP="$(amplevoice_infer_server_ip "$(pwd)" || true)"
fi

[[ -n "${SERVER_IP:-}" ]] || amplevoice_fail "Could not determine SERVER_IP from the existing install"

amplevoice_set_env_key .env SERVER_IP "$SERVER_IP"
amplevoice_set_env_key .env PUBLIC_HOST "$DOMAIN_NAME"
amplevoice_set_env_key .env PUBLIC_BASE_URL "https://$DOMAIN_NAME"
amplevoice_delete_env_key .env BACKEND_URL
amplevoice_prepare_remote_install "$(pwd)"
...
cat > /etc/letsencrypt/renewal-hooks/deploy/amplevoice-reload.sh << HOOK_EOF
#!/bin/bash
cp /etc/letsencrypt/live/$DOMAIN_NAME/fullchain.pem $AMPLEVOICE_PATH/certs/local.crt
cp /etc/letsencrypt/live/$DOMAIN_NAME/privkey.pem $AMPLEVOICE_PATH/certs/local.key
chmod 644 $AMPLEVOICE_PATH/certs/local.crt $AMPLEVOICE_PATH/certs/local.key

cd $AMPLEVOICE_PATH
docker compose --profile remote restart nginx 2>/dev/null || true
HOOK_EOF
chmod +x /etc/letsencrypt/renewal-hooks/deploy/amplevoice-reload.sh
...
echo ""
echo -e "${BLUE}[7/7] Starting AmpleVoiceAI services through validated startup wrapper...${NC}"
./remote_up.sh
...
echo -e "  Certificate: $AMPLEVOICE_PATH/certs/local.crt"
echo -e "  Private Key: $AMPLEVOICE_PATH/certs/local.key"
echo -e "  Auto-renewal: Enabled (certificates renew automatically)"
echo ""
echo -e "${YELLOW}Files modified:${NC}"
echo "  - amplevoice/.env (canonical public host/base URL updated)"
echo "  - amplevoice/certs/local.crt (SSL certificate)"
echo "  - amplevoice/certs/local.key (SSL private key)"
echo "  - /etc/letsencrypt/renewal-hooks/deploy/amplevoice-reload.sh (renewal hook)"
echo ""
