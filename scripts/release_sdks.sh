#!/usr/bin/env bash
# Cut a release of both SDKs — amplevoice-sdk (PyPI) and @amplevoice/sdk (npm) —
...
#   - `npm login` completed as a member of the `amplevoice` npm org. npm
...
if ! NPM_USER="$(npm whoami 2>/dev/null)"; then
    ...
    echo "       amplevoice org before re-running this script — otherwise PyPI" >&2
...
if ! git diff --quiet -- sdk/python/src/amplevoice_sdk/typed sdk/typescript/src/typed; then
    echo
...
echo
echo "============================================================"
echo "  Built amplevoice-sdk==$VERSION and @amplevoice/sdk@$VERSION"
echo "  Nothing has been published yet."
echo "============================================================"
echo

if confirm "Upload amplevoice-sdk==$VERSION to TestPyPI first (recommended)?"; then
    (cd sdk/python && twine upload --repository testpypi dist/*)
    echo "  → https://test.pypi.org/project/amplevoice-sdk/$VERSION/"
    echo
fi

if confirm "Publish @amplevoice/sdk@$VERSION to npm? (will prompt for 2FA OTP)"; then
    (cd sdk/typescript && npm publish --access public)
    echo "  → https://www.npmjs.com/package/@amplevoice/sdk/v/$VERSION"
    echo
fi

if confirm "Upload amplevoice-sdk==$VERSION to PyPI?"; then
    (cd sdk/python && twine upload dist/*)
    echo "  → https://pypi.org/project/amplevoice-sdk/$VERSION/"
    echo
fi

if confirm "Create annotated git tag sdks-v$VERSION at HEAD?"; then
    git tag -a "sdks-v$VERSION" -m "amplevoice-sdk + @amplevoice/sdk $VERSION"
