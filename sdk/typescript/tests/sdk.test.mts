// Unit tests for @amplevoice/sdk. Uses Node's built-in `node:test` runner and
// an in-memory spec stub — no HTTP, no backend dependency. Mirrors the
// Python SDK tests in api/tests/test_amplevoice_sdk.py.
//
...
// ─── AmpleVoiceAIClient HTTP plumbing (stubbed fetch) ───────────────────────────

describe("AmpleVoiceAIClient", () => {
    it("sends the API key as X-API-Key", async () => {
        ...
        const c = new AmpleVoiceAIClient({
