// HTTP client for the AmpleVoiceAI REST API.
//
...
export interface AmpleVoiceAIFetchInit {
    method?: string;
...
export interface AmpleVoiceAIFetchResponse {
    ok: boolean;
    status: number;
    statusText: string;
...
export type AmpleVoiceAIFetch = (
    url: string,
    init?: AmpleVoiceAIFetchInit,
) => Promise<AmpleVoiceAIFetchResponse>;
...
export interface AmpleVoiceAIClientOptions {
    baseUrl?: string;
    apiKey?: string;
    /** Request timeout in ms. */
    timeoutMs?: number;
    /** Optional fetch override for tests / custom transports. */
    fetch?: AmpleVoiceAIFetch;
}

export class AmpleVoiceAIClient extends _GeneratedClient implements SpecProvider {
    readonly baseUrl: string;
    readonly apiKey: string | undefined;
    private readonly fetchImpl: AmpleVoiceAIFetch;
    private readonly timeoutMs: number;
    private readonly headers: Record<string, string>;
    private readonly specCache = new Map<string, NodeSpec>();
    private specVersionCache: string | null = null;

    constructor(opts: AmpleVoiceAIClientOptions = {}) {
        super();
        const rawBase =
            opts.baseUrl ??
            getRuntimeEnv("AMPLEVOICE_API_URL") ??
            "http://localhost:8000";
        this.baseUrl = rawBase.replace(/\/+$/, "");
        this.apiKey = opts.apiKey ?? getRuntimeEnv("AMPLEVOICE_API_KEY");
        this.fetchImpl = opts.fetch ?? (globalThis.fetch as unknown as AmpleVoiceAIFetch);
    ...
    protected async request<T = unknown>(
        ...
        const init: AmpleVoiceAIFetchInit = {
            method,
            headers: {
                ...this.headers,
        ...
        let resp: AmpleVoiceAIFetchResponse;
        try {
