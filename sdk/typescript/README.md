...
Typed builder for AmpleVoiceAI voice-AI workflows. Fetches the node-spec catalog from
the AmpleVoiceAI backend at session start, validates every call against it at the
...
{
  "paths": {
    "@amplevoice/sdk": ["../sdk/typescript/src/index.ts"]
  }
}
```
...
const client = new AmpleVoiceAIClient({
  ...
  apiKey: process.env.AMPLEVOICE_API_KEY,
});
...
```bash
AMPLEVOICE_API_URL=http://localhost:8000   # default
AMPLEVOICE_API_KEY=sk-...                  # sent as X-API-Key
```
