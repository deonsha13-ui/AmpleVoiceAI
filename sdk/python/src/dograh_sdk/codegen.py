...
The generated files live under `amplevoice_sdk.typed` and are committed to
the repository so `pip install amplevoice-sdk` ships typed classes without
requiring a regen step.

Run manually:

    python -m amplevoice_sdk.codegen --api http://localhost:8000 \\
        --out sdk/python/src/amplevoice_sdk/typed

    python -m amplevoice_sdk.codegen --input specs.json \\
        --out ./my_typed
"""
...
Regenerate with `python -m amplevoice_sdk.codegen` against the target
AmpleVoiceAI backend. Source of truth: the backend's model-backed node-spec
...
from amplevoice_sdk.typed._base import TypedNode
'''
...
def _render_init_module(spec_names: list[str]) -> str:
    lines = [
        ...
        "`from amplevoice_sdk.typed import StartCall, AgentNode`.",
        '"""',
        "",
    ]
    exports: list[str] = []
    for spec_name in sorted(spec_names):
        ...
        lines.append(f"from amplevoice_sdk.typed.{module_name} import {class_name}")
        exports.append(class_name)

    lines.append("from amplevoice_sdk.typed._base import TypedNode")
...
def main(argv: list[str] | None = None) -> None:
    parser = argparse.ArgumentParser(
        prog="python -m amplevoice_sdk.codegen",
        description="Generate typed SDK dataclasses from the AmpleVoiceAI node-spec catalog.",
    )
    source = parser.add_mutually_exclusive_group(required=True)
    source.add_argument("--api", help="AmpleVoiceAI backend base URL")
