...
`AmpleVoiceAIClient` mixes in this class to get HTTP methods for every route
...
from amplevoice_sdk._generated_models import (
{imports}
)


class _GeneratedClient:
    # `AmpleVoiceAIClient.__init__` installs `self._request` (see client.py).
...
// Regenerate with `./scripts/generate_sdk.sh`.
//
// `AmpleVoiceAIClient` extends this base to get HTTP methods for every route
