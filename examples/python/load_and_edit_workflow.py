...
from amplevoice_sdk import AmpleVoiceAIClient
from amplevoice_sdk._generated_models import UpdateWorkflowRequest

load_dotenv(Path(__file__).parent / ".env")

# Replace with the numeric ID of an existing agent in your AmpleVoiceAI account.
WORKFLOW_ID = 0
...
def main() -> int:
    api_endpoint = os.environ.get("AMPLEVOICE_API_ENDPOINT", "http://localhost:8000")
    api_token = os.environ.get("AMPLEVOICE_API_TOKEN")

    if not api_token:
        print("AMPLEVOICE_API_TOKEN is required", file=sys.stderr)
        return 1
    ...
    with AmpleVoiceAIClient(base_url=api_endpoint, api_key=api_token) as client:
