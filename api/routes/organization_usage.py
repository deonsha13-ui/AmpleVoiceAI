...
async def create_mps_credit_purchase_url(
    ...
    """Create a checkout URL for organizations using AmpleVoiceAI-managed MPS v2."""
    if DEPLOYMENT_MODE == "oss":
        raise HTTPException(
            status_code=404,
    ...
    try:
        session = await mps_service_key_client.create_credit_purchase_url(
            ...
            billing_details={
                "source": "amplevoice_billing",
                "amplevoice_user_id": str(user.id),
                "amplevoice_provider_id": str(user.provider_id),
            },
        )
    except Exception as exc:
...
async def get_usage_history(
    ...
    try:
        ...
        return {
            "runs": runs,
            "total_amplevoice_tokens": total_tokens,
