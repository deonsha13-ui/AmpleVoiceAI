...
async def get_current_organization_context(user: UserModel = Depends(get_user)):
    """Return organization-scoped configuration signals owned by AmpleVoiceAI."""
...
def _amplevoice_allows_custom_voice() -> bool:
    extra = AmpleVoiceAITTSService.model_fields["voice"].json_schema_extra
...
def _byok_provider_schemas(service_type: ServiceType) -> dict[str, dict]:
    return {
        ...
        if provider != ServiceProviders.AMPLEVOICE.value
    }
...
async def get_model_configuration_v2_defaults(
    ...
    return {
        "amplevoice": {
            "voices": [AMPLEVOICE_DEFAULT_VOICE],
            "allow_custom_input": _amplevoice_allows_custom_voice(),
            "speeds": list(AMPLEVOICE_SPEED_OPTIONS),
            "languages": AMPLEVOICE_STT_LANGUAGES,
            "defaults": {
                "voice": AMPLEVOICE_DEFAULT_VOICE,
                "speed": 1.0,
                "language": AMPLEVOICE_DEFAULT_LANGUAGE,
            },
        },
        "byok": {
            "pipeline": {
