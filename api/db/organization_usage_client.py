...
class OrganizationUsageClient(BaseDBClient):
    ...
    async def get_current_usage(self, organization_id: int) -> dict:
        ...
        async with self.async_session() as session:
            ...
            result = {
                ...
                "used_amplevoice_tokens": cycle.used_amplevoice_tokens,
    ...
    async def get_usage_history(
        ...
        async with self.async_session() as session:
            ...
            for run in runs:
                ...
                total_tokens += amplevoice_tokens
                ...
                run_data = {
                    ...
                    "amplevoice_token_usage": amplevoice_tokens,
    ...
    async def get_daily_usage_breakdown(
        ...
        async with self.async_session() as session:
            ...
            total_minutes = 0
            total_cost_usd = 0
            total_amplevoice_tokens = 0

            for row in daily_usage:
                seconds = row.total_seconds or 0
                minutes = seconds / 60
                cost_usd = seconds * price_per_second_usd
                amplevoice_tokens = cost_usd * 100  # 1 cent = 1 token

                total_minutes += minutes
                total_cost_usd += cost_usd
                total_amplevoice_tokens += amplevoice_tokens
                ...
                breakdown.append(
                    {
                        ...
                        "amplevoice_tokens": round(amplevoice_tokens, 0),
            ...
            return {
                ...
                "total_amplevoice_tokens": round(total_amplevoice_tokens, 0),
                "currency": "USD",
            }
