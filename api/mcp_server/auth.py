...
async def authenticate_mcp_request() -> UserModel:
    """Resolve the authenticated AmpleVoiceAI user for an MCP tool invocation.
    ...
    and user identifiers. `_OrgRoutingExporter` reads `amplevoice.org_id`
    ...
    if span.is_recording():
        ...
        # Intentionally NOT `amplevoice.org_id` — that attribute triggers the
