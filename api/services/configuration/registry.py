...
AMPLEVOICE_PROVIDER_MODEL_CONFIG = provider_model_config("AmpleVoiceAI")
...
AMPLEVOICE_LLM_MODELS = ["default", "accurate", "fast", "lite", "zen"]
AWS_BEDROCK_MODELS = [
...
@register_llm
class AmpleVoiceAILLMService(BaseLLMConfiguration):
    model_config = AMPLEVOICE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AMPLEVOICE] = ServiceProviders.AMPLEVOICE
    model: str = Field(
        default="default",
        description="AmpleVoiceAI-hosted model tier.",
        json_schema_extra={"examples": AMPLEVOICE_LLM_MODELS, "allow_custom_input": True},
    )
...
class GoogleTTSConfiguration(BaseTTSConfiguration):
    ...
    model: str = Field(
        default="chirp_3_hd",
        description=(
            "Google Cloud low-latency TTS engine. AmpleVoiceAI maps this to Pipecat's "
...
AMPLEVOICE_TTS_MODELS = ["default"]


@register_tts
class AmpleVoiceAITTSService(BaseTTSConfiguration):
    model_config = AMPLEVOICE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AMPLEVOICE] = ServiceProviders.AMPLEVOICE
    model: str = Field(
        default="default",
        description="AmpleVoiceAI TTS tier.",
        json_schema_extra={"examples": AMPLEVOICE_TTS_MODELS},
    )
    voice: str = Field(
        default="default",
...
# AmpleVoiceAI STT Service
AMPLEVOICE_STT_MODELS = ["default"]
AMPLEVOICE_STT_LANGUAGES = DEEPGRAM_LANGUAGES


@register_stt
class AmpleVoiceAISTTService(BaseSTTConfiguration):
    model_config = AMPLEVOICE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AMPLEVOICE] = ServiceProviders.AMPLEVOICE
    model: str = Field(
        default="default",
        description="AmpleVoiceAI STT tier.",
        json_schema_extra={"examples": AMPLEVOICE_STT_MODELS},
    )
    language: str = Field(
        default="multi",
        description="Language code; use 'multi' for auto-detect.",
        json_schema_extra={"examples": AMPLEVOICE_STT_LANGUAGES},
    )
...
AMPLEVOICE_EMBEDDING_MODELS = ["default"]


@register_embeddings
class AmpleVoiceAIEmbeddingsConfiguration(BaseEmbeddingsConfiguration):
    model_config = AMPLEVOICE_PROVIDER_MODEL_CONFIG
    provider: Literal[ServiceProviders.AMPLEVOICE] = ServiceProviders.AMPLEVOICE
    model: str = Field(
        default="default",
        description="AmpleVoiceAI-managed embedding model.",
        json_schema_extra={"examples": AMPLEVOICE_EMBEDDING_MODELS},
    )
