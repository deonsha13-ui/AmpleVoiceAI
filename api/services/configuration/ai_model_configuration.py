...
from api.schemas.ai_model_configuration import (
    AMPLEVOICE_DEFAULT_LANGUAGE,
    AMPLEVOICE_DEFAULT_VOICE,
    AMPLEVOICE_SPEED_OPTIONS,
    BYOKAIModelConfiguration,
    BYOKPipelineAIModelConfiguration,
    BYOKRealtimeAIModelConfiguration,
    AmpleVoiceAIManagedAIModelConfiguration,
...
def merge_ai_model_configuration_v2_secrets(
    ...
    if incoming_dict.get("mode") == "amplevoice" and existing_dict.get("mode") == "amplevoice":
        incoming_amplevoice = incoming_dict.get("amplevoice") or {}
        existing_amplevoice = existing_dict.get("amplevoice") or {}
        incoming_key = incoming_amplevoice.get("api_key")
        existing_key = existing_amplevoice.get("api_key")
        if incoming_key and existing_key and contains_masked_key(incoming_key):
            incoming_amplevoice["api_key"] = resolve_masked_api_keys(
                incoming_key,
                existing_key,
            )
...
def convert_legacy_ai_model_configuration_to_v2(
    ...
    amplevoice_key = _first_amplevoice_api_key(configuration)
    if amplevoice_key:
        return _convert_any_amplevoice_legacy_configuration(configuration, amplevoice_key)
...
def amplevoice_embeddings_base_url() -> str:
...
def apply_managed_embeddings_base_url(
    *,
    ...
    if provider == ServiceProviders.AMPLEVOICE.value or provider == ServiceProviders.AMPLEVOICE:
        return amplevoice_embeddings_base_url()
    return base_url
...
def _convert_any_amplevoice_legacy_configuration(
    configuration: EffectiveAIModelConfiguration,
    amplevoice_key: str,
) -> OrganizationAIModelConfigurationV2:
    speed = getattr(configuration.tts, "speed", 1.0)
    if speed not in AMPLEVOICE_SPEED_OPTIONS:
        speed = 1.0
    return OrganizationAIModelConfigurationV2(
        mode="amplevoice",
        amplevoice=AmpleVoiceAIManagedAIModelConfiguration(
            api_key=amplevoice_key,
            voice=getattr(configuration.tts, "voice", AMPLEVOICE_DEFAULT_VOICE)
            or AMPLEVOICE_DEFAULT_VOICE,
            speed=speed,
            language=getattr(configuration.stt, "language", AMPLEVOICE_DEFAULT_LANGUAGE)
            or AMPLEVOICE_DEFAULT_LANGUAGE,
        ),
    )


def _first_amplevoice_api_key(configuration: EffectiveAIModelConfiguration) -> str | None:
    for service in (
        configuration.llm,
        configuration.tts,
        configuration.stt,
        ...
        if service is None or _provider(service) != ServiceProviders.AMPLEVOICE:
            continue
        try:
