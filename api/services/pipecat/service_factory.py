...
from pipecat.services.amplevoice.llm import AmpleVoiceAILLMService
from pipecat.services.amplevoice.stt import AmpleVoiceAISTTService, AmpleVoiceAISTTSettings
from pipecat.services.amplevoice.tts import AmpleVoiceAITTSService, AmpleVoiceAITTSSettings
...
def create_stt_service(
    ...
    elif user_config.stt.provider == ServiceProviders.AMPLEVOICE.value:
        base_url = MPS_API_URL.replace("http://", "ws://").replace("https://", "wss://")
        language = getattr(user_config.stt, "language", None) or "multi"
        return AmpleVoiceAISTTService(
            base_url=base_url,
            api_key=user_config.stt.api_key,
            correlation_id=correlation_id,
            settings=AmpleVoiceAISTTSettings(
                model=user_config.stt.model,
                language=language,
            ),
            keyterms=keyterms,
...
def create_tts_service(
    ...
    elif user_config.tts.provider == ServiceProviders.AMPLEVOICE.value:
        # Convert HTTP URL to WebSocket URL for TTS
        base_url = MPS_API_URL.replace("http://", "ws://").replace("https://", "wss://")
        return AmpleVoiceAITTSService(
            base_url=base_url,
            api_key=user_config.tts.api_key,
            correlation_id=correlation_id,
            settings=AmpleVoiceAITTSSettings(
                model=user_config.tts.model,
                voice=user_config.tts.voice,
                speed=user_config.tts.speed,
            ),
...
def create_llm_service_from_provider(
    ...
    *,
    ...
    elif provider == ServiceProviders.AMPLEVOICE.value:
        return AmpleVoiceAILLMService(
...
def create_realtime_llm_service(user_config, audio_config: "AudioConfig"):
    ...
    if provider == ServiceProviders.OPENAI_REALTIME.value:
        from api.services.pipecat.realtime.openai_realtime import (
            AmpleVoiceAIOpenAIRealtimeLLMService,
        )
        ...
        return AmpleVoiceAIOpenAIRealtimeLLMService(
            api_key=api_key,
            settings=AmpleVoiceAIOpenAIRealtimeLLMService.Settings(
                model=model,
        ...
        from api.services.pipecat.realtime.grok_realtime import (
            AmpleVoiceAIGrokRealtimeLLMService,
        )
        from pipecat.services.xai.realtime.events import SessionProperties

        return AmpleVoiceAIGrokRealtimeLLMService(
            api_key=api_key,
            settings=AmpleVoiceAIGrokRealtimeLLMService.Settings(
                model=model,
                session_properties=SessionProperties(
                    voice=voice or "Ara",
                ),
        ...
        from api.services.pipecat.realtime.ultravox_realtime import (
            AmpleVoiceAIUltravoxOneShotInputParams,
            AmpleVoiceAIUltravoxRealtimeLLMService,
        )

        return AmpleVoiceAIUltravoxRealtimeLLMService(
            params=AmpleVoiceAIUltravoxOneShotInputParams(
                api_key=api_key,
                model=model,
                voice=voice,
                output_medium="voice",
            ),
            settings=AmpleVoiceAIUltravoxRealtimeLLMService.Settings(
                model=model,
                output_medium="voice",
            ),
        )
        ...
        return AmpleVoiceAIGeminiLiveLLMService(
            api_key=api_key,
            settings=AmpleVoiceAIGeminiLiveLLMService.Settings(**settings_kwargs),
        )
        ...
        if language:
            settings_kwargs["language"] = language
        return AmpleVoiceAIGeminiLiveVertexLLMService(
            credentials=credentials,
            project_id=project_id,
            location=location,
            settings=AmpleVoiceAIGeminiLiveVertexLLMService.Settings(**settings_kwargs),
        )
    elif provider == ServiceProviders.AZURE_REALTIME.value:
        from api.services.pipecat.realtime.azure_realtime import (
            AmpleVoiceAIAzureRealtimeLLMService,
        )
        from pipecat.services.openai.realtime.events import (
            AudioConfiguration,
            AudioInput,
        ...
        return AmpleVoiceAIAzureRealtimeLLMService(
            api_key=api_key,
            base_url=wss_url,
            settings=AmpleVoiceAIAzureRealtimeLLMService.Settings(
                model=model,
                session_properties=SessionProperties(
                    audio=AudioConfiguration(
                        input=AudioInput(
