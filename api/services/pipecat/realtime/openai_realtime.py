"""AmpleVoiceAI subclass of pipecat's OpenAI Realtime LLM service.

Layers AmpleVoiceAI engine integration quirks onto upstream-pristine
...
- **One-off LLMMessagesAppendFrame handling** for ephemeral realtime prompts
  like user-idle checks, without mutating AmpleVoiceAI's local ``LLMContext``.
...
class AmpleVoiceAIOpenAIRealtimeLLMService(OpenAIRealtimeLLMService):
    """OpenAI Realtime with AmpleVoiceAI engine integration quirks. See module docstring."""

    def __init__(self, **kwargs):
        super().__init__(**kwargs)
        self._user_is_muted: bool = False
        # AmpleVoiceAI pre-populates self._context via the engine before the first
        ...
        # has finished speaking, matching AmpleVoiceAI's Gemini Live behavior.
    ...
    async def process_frame(self, frame: Frame, direction: FrameDirection):
        ...
        if isinstance(frame, TTSSpeakFrame):
            ...
            # lifecycle even when AmpleVoiceAI has already pre-populated self._context.
