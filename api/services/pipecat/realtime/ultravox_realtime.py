"""AmpleVoiceAI subclass of pipecat's Ultravox realtime LLM service.

Ultravox is audio-native and realtime, but prompt and tool configuration is
bound to call creation. AmpleVoiceAI therefore cannot lean on in-session updates or
Gemini-style session resumption handles. This wrapper adapts Ultravox to the
AmpleVoiceAI engine contract by:
...
- reconstructing Ultravox ``initialMessages`` from AmpleVoiceAI context when the
  call must be recreated after a node transition
- appending a transient resumptive user nudge to recreated ``initialMessages``
  after tool-result transitions, without mutating AmpleVoiceAI's stored context
- handling AmpleVoiceAI-only frames such as user mute and idle append prompts
...
class AmpleVoiceAIUltravoxOneShotInputParams(OneShotInputParams):
    """AmpleVoiceAI-friendly OneShot params with string voice support."""
...
class AmpleVoiceAIUltravoxRealtimeLLMService(UltravoxRealtimeLLMService):
    """Ultravox realtime with AmpleVoiceAI engine integration quirks."""
    ...
    async def start(self, frame):
        # AmpleVoiceAI defers call creation until the engine queues the node opening.
    ...
    def _build_one_shot_params(
        ...
        *,
        ...
        return AmpleVoiceAIUltravoxOneShotInputParams(
