...
async def create_transport(
    ...
    *,
    ...
    # not AmpleVoiceAI's: ``inbound_encoding`` is what we *send into the call*
    # (AmpleVoiceAI → Telnyx), and ``outbound_encoding`` is what we *receive out of
    # the call* (Telnyx → AmpleVoiceAI).
    serializer = TelnyxFrameSerializer(
        stream_id=stream_id,
        call_control_id=call_control_id,
        api_key=api_key,
        inbound_encoding="PCMU",  # AmpleVoiceAI → Telnyx; matches stream_bidirectional_codec
        outbound_encoding=encoding,  # Telnyx → AmpleVoiceAI; from media_format.encoding
