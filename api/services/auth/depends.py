...
from api.constants import AUTH_PROVIDER, AMPLEVOICE_MPS_SECRET_KEY, MPS_API_URL
    ...
    async with httpx.AsyncClient() as client:
        ...
        if AUTH_PROVIDER == "local":
            ...
            response = await client.post(
                ...
                json={
                    "name": "Default AmpleVoiceAI Model Service Key",
            ...
            if not AMPLEVOICE_MPS_SECRET_KEY:
                logger.warning(
                    "Warning: AMPLEVOICE_MPS_SECRET_KEY not set for authenticated mode"
                )
                raise ValidationError("Missing AMPLEVOICE_MPS_SECRET_KEY in non oss mode")
            ...
            response = await client.post(
                ...
                headers={"X-Secret-Key": AMPLEVOICE_MPS_SECRET_KEY},
                timeout=10.0,
            )
        ...
        if response.status_code == 200:
            ...
            if service_key:
                ...
                configuration = {
                    "llm": {
                        "provider": ServiceProviders.AMPLEVOICE.value,
                        "api_key": [service_key],
                        "model": "default",
                    },
                    "tts": {
