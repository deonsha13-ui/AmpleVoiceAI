"""Quota checking service for AmpleVoiceAI credits.
...
def _service_uses_amplevoice(service: Any) -> bool:
    provider = getattr(service, "provider", None)
    return (
        provider == ServiceProviders.AMPLEVOICE or provider == ServiceProviders.AMPLEVOICE.value
    )


def _amplevoice_api_keys(user_config: Any) -> set[str]:
    api_keys: set[str] = set()
    for section_name in ("llm", "stt", "tts", "embeddings"):
        service = getattr(user_config, section_name, None)
        if not _service_uses_amplevoice(service):
            continue
...
async def _authorize_hosted_workflow_run_start(
    *,
    ...
    service_key = (
        get_amplevoice_service_api_key(user_config) if requires_correlation else None
    )
    ...
    try:
        authorization = await mps_service_key_client.authorize_workflow_run_start(
            ...
            minimum_credits=MINIMUM_AMPLEVOICE_CREDITS_FOR_CALL,
            created_by=(
            ...
            metadata={
                "amplevoice_user_id": str(workflow_owner.id),
        ...
        return (
            QuotaCheckResult(
                ...
                error_message="Could not verify AmpleVoiceAI credits. Please try again.",
            ),
            True,
        )
    ...
    if (
        ...
        or remaining < MINIMUM_AMPLEVOICE_CREDITS_FOR_CALL
    ):
        logger.warning(
            "Insufficient AmpleVoiceAI billing v2 credits for org {}: {:.2f} credits remaining",
            organization_id,
            remaining,
        )
    ...
    logger.info(
        "AmpleVoiceAI billing v2 run authorization passed for org {}: {:.2f} credits remaining",
        organization_id,
        remaining,
    )
...
async def _authorize_legacy_amplevoice_keys(
    *,
    amplevoice_api_keys: set[str],
    organization_id: int | None,
    workflow_owner: UserModel,
) -> QuotaCheckResult:
    for api_key in amplevoice_api_keys:
        try:
            ...
            if remaining < MINIMUM_AMPLEVOICE_CREDITS_FOR_CALL:
                logger.warning(
                    f"Insufficient AmpleVoiceAI credits for key ...{api_key[-8:]}: "
                    f"${remaining:.2f} remaining"
                )
                return _insufficient_legacy_quota_result()

            logger.info(
                f"AmpleVoiceAI quota check passed for key ...{api_key[-8:]}: "
                f"{remaining:.2f} credits remaining"
            )
        except Exception as e:
            logger.error(f"Failed to check quota for AmpleVoiceAI key: {str(e)}")
            error_str = str(e)
...
async def _authorize_oss_managed_v2_correlation(
    *,
    ...
    service_key = get_amplevoice_service_api_key(user_config)
    if not service_key:
        return QuotaCheckResult(
            has_quota=False,
            error_code="invalid_service_key",
...
async def authorize_workflow_run_start(
    *,
    ...
    try:
        ...
        amplevoice_api_keys = _amplevoice_api_keys(user_config)
        if not amplevoice_api_keys:
            return QuotaCheckResult(has_quota=True)

        legacy_result = await _authorize_legacy_amplevoice_keys(
            amplevoice_api_keys=amplevoice_api_keys,
            organization_id=(
