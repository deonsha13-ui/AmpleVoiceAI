"""Pydantic schemas for reusable AmpleVoiceAI tools.
...
class PresetToolParameter(BaseModel):
    """A parameter injected by AmpleVoiceAI at runtime."""
...
class HttpApiConfig(BaseModel):
    ...
    preset_parameters: Optional[List[PresetToolParameter]] = Field(
        default=None,
        description=(
            "Parameters injected by AmpleVoiceAI from fixed values or workflow context "
            "templates."
        ),
    )
