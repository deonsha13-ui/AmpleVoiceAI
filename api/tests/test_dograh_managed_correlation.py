...
from pipecat.services.amplevoice.stt import AmpleVoiceAISTTService
from pipecat.services.amplevoice.tts import AmpleVoiceAITTSService
...
def test_amplevoice_llm_uses_explicit_mps_correlation_id():
    service = AmpleVoiceAILLMService(
        api_key="mps-secret",
...
@pytest.mark.asyncio
async def test_amplevoice_stt_config_uses_explicit_mps_correlation_id(monkeypatch):
    ...
    monkeypatch.setattr(
        "pipecat.services.amplevoice.stt.websocket_connect",
        fake_connect,
    )

    service = AmpleVoiceAISTTService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=16000,
    )
...
@pytest.mark.asyncio
async def test_amplevoice_tts_messages_use_explicit_mps_correlation_id(monkeypatch):
    fake_ws = _FakeWebSocket()
    ...
    monkeypatch.setattr(
        "pipecat.services.amplevoice.tts.websocket_connect",
        fake_connect,
    )

    service = AmpleVoiceAITTSService(
        api_key="mps-secret",
        correlation_id="mps-corr-123",
        sample_rate=24000,
    )
...
@pytest.mark.asyncio
async def test_amplevoice_tts_final_for_missing_context_is_ignored():
    service = AmpleVoiceAITTSService(api_key="mps-secret")
