...
def test_amplevoice_v2_compiles_to_effective_managed_pipeline_with_embeddings():
    ...
    assert effective.llm.provider == "amplevoice"
    assert effective.llm.model == "default"
    assert effective.tts.provider == "amplevoice"
    assert effective.tts.speed == 1.2
    assert effective.stt.provider == "amplevoice"
    assert effective.stt.language == "multi"
    assert effective.embeddings.provider == "amplevoice"
    assert effective.embeddings.model == "default"
    assert effective.managed_service_version == 2


def test_amplevoice_v2_rejects_non_predefined_speed():
...
def test_byok_v2_rejects_amplevoice_provider():
    with pytest.raises(ValidationError):
...
def test_masked_amplevoice_key_is_preserved_when_saving_same_mode():
    existing = OrganizationAIModelConfigurationV2(
        mode="amplevoice",
        amplevoice=AmpleVoiceAIManagedAIModelConfiguration(api_key="mps-real-secret"),
    )
    incoming = OrganizationAIModelConfigurationV2(
        mode="amplevoice",
        amplevoice=AmpleVoiceAIManagedAIModelConfiguration(api_key=mask_key("mps-real-secret")),
    )

    merged = merge_ai_model_configuration_v2_secrets(incoming, existing)

    assert merged.amplevoice.api_key == "mps-real-secret"
...
def test_legacy_all_amplevoice_pipeline_converts_to_amplevoice_v2():
    ...
    assert config.mode == "amplevoice"
    assert config.amplevoice.api_key == "mps-secret"


def test_legacy_mixed_amplevoice_pipeline_converts_to_amplevoice_v2():
    legacy = EffectiveAIModelConfiguration(
    ...
    assert config.mode == "amplevoice"
    assert config.amplevoice.api_key == "mps-tts"
    assert config.amplevoice.voice == "default"
...
def test_workflow_model_override_migration_removes_v1_override_and_sets_v2():
    ...
    assert v2_override["mode"] == "amplevoice"
    assert v2_override["amplevoice"]["api_key"] == "mps-workflow"
...
async def test_migrate_model_configuration_v2_initializes_hosted_mps_billing(
    ...
    expected_response = OrganizationAIModelConfigurationResponse(
        configuration={"version": 2, "mode": "amplevoice"},
