"""Tests for the Python runtime SDK (`amplevoice_sdk`).
...
import pytest
from amplevoice_sdk import Workflow
from amplevoice_sdk._generated_models import NodeSpec
from amplevoice_sdk.errors import ValidationError
...
class _StubClient:
    """Stand-in for AmpleVoiceAIClient backed by the in-process spec registry.
...
def test_webhook_complex_fields_validate(client: _StubClient):
    ...
    wh = wf.add(
        ...
        custom_headers=[{"key": "X-Source", "value": "amplevoice"}],
