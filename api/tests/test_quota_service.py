...
def _amplevoice_config(
    ...
    *,
    ...
    return SimpleNamespace(
        ...
        llm=SimpleNamespace(provider=ServiceProviders.AMPLEVOICE, api_key=api_key),
        stt=None,
        tts=None,
        embeddings=None,
    )
...
async def test_authorize_workflow_run_uses_workflow_org_for_hosted_v2(
    ...
    get_config = AsyncMock(return_value=_amplevoice_config())
    authorize = AsyncMock(
        return_value={
            "allowed": True,
            "billing_mode": "v2",
    ...
    authorize.assert_awaited_once_with(
        ...
        minimum_credits=quota_service.MINIMUM_AMPLEVOICE_CREDITS_FOR_CALL,
        created_by="provider-123",
        metadata={"amplevoice_user_id": "123", "workflow_id": 7},
    )
...
async def test_authorize_workflow_run_v2_insufficient_credits_prompts_billing(
    ...
    assert "founders@amplevoice.com" not in result.error_message
...
async def test_authorize_workflow_run_v1_uses_legacy_key_usage(
    ...
    get_config = AsyncMock(return_value=_amplevoice_config(api_key))
    authorize = AsyncMock(
        return_value={
            "allowed": True,
            "billing_mode": "v1",
    ...
    assert "founders@amplevoice.com" in result.error_message
...
async def test_authorize_workflow_run_managed_v2_stores_hosted_correlation(
    ...
    get_config = AsyncMock(
        return_value=_amplevoice_config(api_key, managed_service_version=2)
    )
    authorize = AsyncMock(
        return_value={
            "allowed": True,
