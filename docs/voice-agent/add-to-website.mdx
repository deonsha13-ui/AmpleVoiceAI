...
description: Add your AmpleVoiceAI voice agent to any website so visitors can talk to it.
tag: "NEW"
---
...
| **Inline Component**  | A panel rendered inside a `<div id="amplevoice-inline-container">` that you place in your page.        | You want the agent embedded in a specific section (landing-page hero, support tab, etc.).            |
| **Headless**          | No UI. Only the audio pipeline plus a JavaScript API on `window.AmpleVoiceAIWidget`.                     | You want full control over the UI — your own buttons, design system, framework state, animations.    |
...
- The embed snippet you copy from the dashboard is a single `<script>` tag that loads `amplevoice-widget.js` **asynchronously**. The widget auto-initializes once it loads and exposes `window.AmpleVoiceAIWidget`. Code that registers callbacks must wait for the widget to be available.
...
```html
<!-- Paste the amplevoice embed snippet from the dashboard somewhere on the page -->
<div id="amplevoice-inline-container"></div>
```

### React

Because React mounts after the widget script may have already loaded, integrate via `initInline` on first mount and `refresh` on remount. Poll for `window.AmpleVoiceAIWidget` to handle the async script load.
...
export function Assistant() {
  useEffect(() => {
    let retries = 0;
    const tryInit = () => {
      const container = document.getElementById('amplevoice-inline-container');
      if (window.AmpleVoiceAIWidget && container) {
        const { isInitialized } = window.AmpleVoiceAIWidget.getState();
        if (isInitialized) window.AmpleVoiceAIWidget.refresh();
        else window.AmpleVoiceAIWidget.initInline({ container });
      } else if (retries++ < 50) {
  ...
  return <div id="amplevoice-inline-container" />;
}
```
...
| ---------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `window.AmpleVoiceAIWidget.start()`                  | Begin a voice call. Must be called from inside a user-gesture handler (e.g. `click`) so the browser grants microphone access.                                |
| `window.AmpleVoiceAIWidget.end()`                    | End the active call.                                                                                                                                         |
| `window.AmpleVoiceAIWidget.onCallStart(cb)`          | Fires when `start()` is invoked (status `connecting`). No payload.                                                                                           |
| `window.AmpleVoiceAIWidget.onCallConnected(cb)`      | Fires when the WebRTC connection is established. Payload: `{ agentId, workflowRunId, token }`.                                                               |
| `window.AmpleVoiceAIWidget.onCallDisconnected(cb)`   | Fires only if the call had connected, when teardown runs. Payload: `{ agentId, workflowRunId, token, durationSeconds }`.                                     |
| `window.AmpleVoiceAIWidget.onCallEnd(cb)`            | Fires whenever the call session is torn down (including failed-to-connect attempts). No payload.                                                             |
| `window.AmpleVoiceAIWidget.onStatusChange(cb)`       | Fires on every status change. Callback receives `(status, text, subtext)`. Status values: `idle`, `connecting`, `connected`, `failed`.                       |
| `window.AmpleVoiceAIWidget.onError(cb)`              | Fires on errors (mic permission denied, server error, etc.). Callback receives an `Error` object.                                                            |

All `on*` setters are single-listener — calling the same one again replaces the previous handler.

<Note>
**About timing.** The widget script loads asynchronously, so `window.AmpleVoiceAIWidget` may not exist at the moment your inline `<script>` first runs. The examples below assume `window.AmpleVoiceAIWidget` is already available when registration runs. To guarantee that:
...
<script>
  ...
  window.AmpleVoiceAIWidget.onError((err) => {
    console.error('AmpleVoiceAI error:', err.message);
  });
...
export function TalkButton() {
  ...
  useEffect(() => {
    window.AmpleVoiceAIWidget.onStatusChange((s) => setStatus(s));
    window.AmpleVoiceAIWidget.onError((err) => console.error('AmpleVoiceAI error:', err.message));
  }, []);
...
```js
window.AmpleVoiceAIWidget.onCallConnected(({ agentId, workflowRunId }) => {
  analytics.track('voice_call_started', { agentId, workflowRunId });
});

window.AmpleVoiceAIWidget.onCallDisconnected(({ workflowRunId, durationSeconds }) => {
