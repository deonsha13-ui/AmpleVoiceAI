...
| Field | Description |
| --- | --- |
| **Endpoint URL** | The URL AmpleVoiceAI will send the POST request to. |
...
AmpleVoiceAI sends a `POST` request with the following JSON payload:
...
```javascript
app.post("/amplevoice/pre-call", async (req, res) => {
