...
description: "Trigger a AmpleVoiceAI voice agent to call a phone number from the SDK"
---

Use the SDK to place a test outbound call from a specific agent to a phone number. This is the same endpoint used by the **Test Call** button in the AmpleVoiceAI UI.

## Prerequisites

- A AmpleVoiceAI [API key](/configurations/api-keys) exported as `AMPLEVOICE_API_KEY`
...
with AmpleVoiceAIClient(api_key="YOUR_API_KEY") as client:
...
const client = new AmpleVoiceAIClient({ apiKey: "YOUR_API_KEY" });
