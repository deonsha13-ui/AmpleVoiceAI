---
title: "Update"
description: "Update your self-hosted AmpleVoiceAI stack to a newer image version"
---

This guide covers updating a AmpleVoiceAI stack you've already deployed with [Docker](/deployment/docker) or a [custom domain](/deployment/custom-domain). You run commands from the same directory that contains your `docker-compose.yaml` (this is the `amplevoice/` directory if you used `setup_remote.sh`).
...
AmpleVoiceAI publishes two images — `amplevoice-api` and `amplevoice-ui` — to both container registries:

- **GitHub Container Registry** — [github.com/orgs/amplevoice-hq/packages](https://github.com/orgs/amplevoice-hq/packages)
- **Docker Hub** — [hub.docker.com/u/amplevoiceai](https://hub.docker.com/u/amplevoiceai)
...
|-------|-----------|---------|-------------|
| GitHub release tag | `amplevoice-vX.Y.Z` | `amplevoice-v1.28.0` | What you see at [github.com/amplevoice-hq/amplevoice/releases](https://github.com/amplevoice-hq/amplevoice/releases) |
...
<Warning>
Always update **`amplevoice-api`** and **`amplevoice-ui`** to the **same tag**. The two images are built from the same commit and the UI expects API responses in a matching shape — mixing versions will break the app. `update_remote.sh` handles this for you automatically.
</Warning>
...
- Synchronizes the canonical remote keys in `.env` and validates the runtime config that `amplevoice-init` will render from it.
...
```bash
cd amplevoice
curl -o update_remote.sh https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/update_remote.sh
bash update_remote.sh
```

You'll be prompted for the target version, defaulting to the most recent release. Accepted forms: bare semver (`1.28.0`), v-prefixed (`v1.28.0`), the full GitHub tag (`amplevoice-v1.28.0`), or `main` for bleeding edge — the script normalizes them. Non-interactive callers can set it via environment variable and skip the confirmation prompt:

```bash
TARGET_VERSION=1.28.0 AMPLEVOICE_UPDATE_YES=1 bash update_remote.sh
```
...
<CodeGroup>
```bash macOS/Linux
curl -o start_docker.sh https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/start_docker.sh && chmod +x start_docker.sh
docker compose down
./start_docker.sh
```
```powershell Windows
...
```bash
cd amplevoice
for f in docker-compose.yaml nginx.conf turnserver.conf .env remote_up.sh scripts/run_amplevoice_init.sh scripts/lib/setup_common.sh deploy/templates/nginx.remote.conf.template deploy/templates/turnserver.remote.conf.template; do
...
<Warning>
Rolling back across a database migration is not always safe — if the newer release ran a schema migration, downgrading may leave the DB in a state the older API doesn't understand. If in doubt, [open an issue](https://github.com/amplevoice-hq/amplevoice/issues) before rolling back.
</Warning>
...
# Track latest main:
git pull
# Or pin to a specific release (git tag format is amplevoice-vX.Y.Z):
git checkout amplevoice-v1.28.0
