---
title: "Docker"
description: "Deploy AmpleVoiceAI AI using Docker for local development and remote server environments"
---

AmpleVoiceAI AI can be deployed using Docker in two main configurations. Choose the option that best fits your needs:

- **Option 1**: For local development and testing on your own machine
- **Option 2**: For remote server deployment with HTTPS (using IP address). If you also have a custom domain, you can first deploy AmpleVoiceAI stack on your server using steps in this document and then proceed to the [Custom Domain](deployment/custom-domain) section.

<Tip>
**Using Claude Code or Codex?** Install the official AmpleVoiceAI setup skill and let your agent drive either deployment below — it orients to your OS, picks local vs remote, runs AmpleVoiceAI's own setup scripts, and verifies the result with a built-in health check.
...
Start a new session, then ask it to *"set up AmpleVoiceAI"* (or run `/amplevoice-setup` in Claude Code). More at [amplevoice-hq/amplevoice-plugins](https://github.com/amplevoice-hq/amplevoice-plugins).
</Tip>

## Option 1: Local Docker Deployment

Watch the video tutorial below for a step-by-step walkthrough of setting up AmpleVoiceAI AI locally with Docker.

<iframe
  className="w-full aspect-video rounded-xl"
  src="https://www.tella.tv/video/amplevoice-local-setup-with-docker-5wsf/embed?b=1&title=1&a=1&loop=0&t=0&muted=0&wt=1"
  title="AmpleVoiceAI Local Setup with Docker"
  allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture"
  allowFullScreen
></iframe>

For local development and testing, you can run AmpleVoiceAI AI directly on your machine using Docker with a small startup script.

### Quick Start

Download the compose file and starter script, then confirm the prompt to start AmpleVoiceAI AI:
...
```
```powershell Windows
Invoke-WebRequest -OutFile docker-compose.yaml https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/docker-compose.yaml
Invoke-WebRequest -OutFile start_docker.ps1 https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/start_docker.ps1
.\start_docker.ps1
```
</CodeGroup>
...
<CodeGroup>
```bash macOS/Linux
curl -o setup_local.sh https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/setup_local.sh && chmod +x setup_local.sh && ./setup_local.sh
```
```powershell Windows
Invoke-WebRequest -OutFile setup_local.ps1 https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/setup_local.ps1
.\setup_local.ps1
```
</CodeGroup>
...
It creates `docker-compose.yaml`, a `.env` file with JWT and TURN credentials, and the small helper bundle that `amplevoice-init` uses to render coturn config at startup. Start the stack with the `local-turn` profile so coturn comes up alongside the other services:
...
```bash
curl -o setup_remote.sh https://raw.githubusercontent.com/amplevoice-hq/amplevoice/main/scripts/setup_remote.sh && chmod +x setup_remote.sh && ./setup_remote.sh
```
...
- Validate the runtime config that `amplevoice-init` will render from `.env`
...
After the setup script completes, start AmpleVoiceAI. The script prints the exact command to run at the end — it differs slightly between modes:
...
The setup script creates the following files in the `amplevoice/` directory:
...
| `scripts/run_amplevoice_init.sh` | One-shot init renderer/validator used by Docker Compose |
...
1. Clones the repo into `amplevoice/` (default: `amplevoice-hq/amplevoice@main`; the script will ask for an `owner/name` and `branch` so a fork can point at its own).
2. If you run the script from inside an existing amplevoice checkout, offers to use that checkout instead of cloning.
