...
description: Set up AmpleVoiceAI directly on your host without the devcontainer.
---
...
1. Run the contributor bootstrap. It configures `origin` as your fork and `upstream` as `amplevoice-hq/amplevoice`, initializes the pipecat submodule, creates the Python venv, and copies the `.env` templates.
<CodeGroup>
```bash macOS/Linux
