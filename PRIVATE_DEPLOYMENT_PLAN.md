...
A step-by-step guide to deploying a voice AI stack (AmpleVoiceAI + Claude/Gemini + STT/TTS) entirely within a customer's GCP project, with no data leaving their VPC.
...
- A **GKE cluster** running AmpleVoiceAI (orchestration + Pipecat pipeline).
...
│  │   │  (AmpleVoiceAI pods)   │         │  192.168.255.230         │ ──▶ Cloud STT
...
This is the single piece of plumbing that gives AmpleVoiceAI private access to Vertex AI (Claude/Gemini), Cloud STT, and Cloud TTS.

### 1.1 Set variables and enable APIs

```bash
export PROJECT_ID=$(gcloud config get-value project)
export NETWORK=amplevoice-vpc
...
gcloud compute networks subnets create amplevoice-subnet \
  --network=$NETWORK \
  --range=10.0.0.0/20 \
  --region=$REGION \
...
```bash
gcloud compute addresses create amplevoice-psc-ip \
  --global \
  --purpose=PRIVATE_SERVICE_CONNECT \
  --addresses=$PSC_IP \
  --network=$NETWORK

gcloud compute forwarding-rules create amplevoice-psc-googleapis \
  --global \
  --network=$NETWORK \
  --address=amplevoice-psc-ip \
...
## Phase 3 — Deploy AmpleVoiceAI on GKE in the same VPC

### 3.1 Create a private GKE cluster

```bash
gcloud container clusters create amplevoice-cluster \
  --region=$REGION \
  --network=$NETWORK \
  --subnetwork=amplevoice-subnet \
...
gcloud container clusters get-credentials amplevoice-cluster --region=$REGION
```

### 3.2 Install AmpleVoiceAI via Helm

```bash
helm install amplevoice ./charts/amplevoice -n amplevoice --create-namespace
```

AmpleVoiceAI pods inherit the VPC's DNS, so any call to `aiplatform.googleapis.com`, `speech.googleapis.com`, or `texttospeech.googleapis.com` automatically routes through the PSC endpoint.
...
kubectl create serviceaccount amplevoice-ksa -n amplevoice

# GCP service account
gcloud iam service-accounts create amplevoice-gsa

# Grant Vertex AI access
gcloud projects add-iam-policy-binding $PROJECT_ID \
  --member="serviceAccount:amplevoice-gsa@$PROJECT_ID.iam.gserviceaccount.com" \
  --role="roles/aiplatform.user"

# Bind KSA to GSA
gcloud iam service-accounts add-iam-policy-binding \
  amplevoice-gsa@$PROJECT_ID.iam.gserviceaccount.com \
  --member="serviceAccount:$PROJECT_ID.svc.id.goog[amplevoice/amplevoice-ksa]" \
  --role="roles/iam.workloadIdentityUser"

# Annotate KSA
kubectl annotate serviceaccount amplevoice-ksa -n amplevoice \
  iam.gke.io/gcp-service-account=amplevoice-gsa@$PROJECT_ID.iam.gserviceaccount.com
```
...
gcloud access-context-manager perimeters create amplevoice-perimeter \
  --title="AmpleVoiceAI VPC-SC Perimeter" \
...
gcloud kms keyrings create amplevoice-keyring --location=$REGION
gcloud kms keys create amplevoice-key \
  --keyring=amplevoice-keyring \
  --location=$REGION \
  --purpose=encryption
```
...
Run these from a AmpleVoiceAI pod inside the cluster:
...
gcloud compute networks subnets create amplevoice-subnet --network=$NETWORK \
  --range=10.0.0.0/20 --region=$REGION --enable-private-ip-google-access

# 4. PSC endpoint
gcloud compute addresses create amplevoice-psc-ip --global \
  --purpose=PRIVATE_SERVICE_CONNECT --addresses=$PSC_IP --network=$NETWORK
gcloud compute forwarding-rules create amplevoice-psc-googleapis --global \
  --network=$NETWORK --address=amplevoice-psc-ip --target-google-apis-bundle=all-apis
...
# 6. GKE
gcloud container clusters create amplevoice-cluster --region=$REGION \
  --network=$NETWORK --subnetwork=amplevoice-subnet \
