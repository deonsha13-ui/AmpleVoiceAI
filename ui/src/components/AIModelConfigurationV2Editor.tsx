...
type ModelMode = "realtime" | "amplevoice" | "byok";

interface AmpleVoiceAIDefaults {
    voices: string[];
...
export interface ModelConfigurationDefaultsV2 {
    amplevoice: AmpleVoiceAIDefaults;
    byok: {
...
interface AmpleVoiceAIFormState {
    api_key: string;
    voice: string;
    speed: number;
    language: string;
...
function isAmpleVoiceAIEffectiveConfig(config: Record<string, unknown> | null | undefined): boolean {
    if (!config || config.is_realtime) return false;
    const llm = asRecord(config.llm);
    const tts = asRecord(config.tts);
    const stt = asRecord(config.stt);
    return llm?.provider === "amplevoice" && tts?.provider === "amplevoice" && stt?.provider === "amplevoice";
}
...
function getByokInitialConfig(
    ...
    if (configuration?.mode === "amplevoice" || isAmpleVoiceAIEffectiveConfig(effectiveConfiguration)) {
...
function buildAmpleVoiceAIState(
    defaults: ModelConfigurationDefaultsV2,
    configuration: Record<string, unknown> | null,
    effectiveConfiguration: Record<string, unknown> | null,
): AmpleVoiceAIFormState {
    const fallback = defaults.amplevoice.defaults;
    const configuredAmpleVoiceAI = configuration?.mode === "amplevoice" ? asRecord(configuration.amplevoice) : null;
    if (configuredAmpleVoiceAI) {
        return {
            api_key: String(configuredAmpleVoiceAI.api_key || ""),
            voice: String(configuredAmpleVoiceAI.voice || fallback.voice),
            speed: Number(configuredAmpleVoiceAI.speed || fallback.speed),
            language: String(configuredAmpleVoiceAI.language || fallback.language),
        };
    }

    if (isAmpleVoiceAIEffectiveConfig(effectiveConfiguration)) {
...
function preferredMode(
    ...
    if (configuration?.mode === "amplevoice") return "amplevoice";
    if (configuration?.mode === "byok") {
        return asRecord(configuration.byok)?.mode === "realtime" ? "realtime" : "byok";
    }
    if (isAmpleVoiceAIEffectiveConfig(effectiveConfiguration)) return "amplevoice";
...
function requireByokService(
    ...
    if (
        ...
        || serviceConfiguration.provider === "amplevoice"
...
function optionalByokService(config: Record<string, unknown>, service: ServiceSegment): Record<string, unknown> | undefined {
    const serviceConfiguration = asRecord(config[service]);
    if (!serviceConfiguration?.provider || serviceConfiguration.provider === "amplevoice") return undefined;
    return serviceConfiguration;
}
...
export function AIModelConfigurationV2Editor({
    ...
    const [mode, setMode] = useState<ModelMode>("amplevoice");
    const [amplevoice, setAmpleVoiceAI] = useState<AmpleVoiceAIFormState>(() => ({
        api_key: "",
        voice: defaults.amplevoice.defaults.voice,
        speed: defaults.amplevoice.defaults.speed,
        language: defaults.amplevoice.defaults.language,
    }));
    ...
    const [isSavingAmpleVoiceAI, setIsSavingAmpleVoiceAI] = useState(false);
    const [isCustomVoice, setIsCustomVoice] = useState(false);
    const [error, setError] = useState<string | null>(null);

    const allowCustomVoice = defaults.amplevoice.allow_custom_input ?? false;
    ...
    useEffect(() => {
        ...
        const nextAmpleVoiceAI = buildAmpleVoiceAIState(defaults, rawConfiguration, rawEffectiveConfiguration);
        setAmpleVoiceAI(nextAmpleVoiceAI);
        setIsCustomVoice(allowCustomVoice && !defaults.amplevoice.voices.includes(nextAmpleVoiceAI.voice));
    ...
    const saveAmpleVoiceAIConfiguration = async () => {
        setIsSavingAmpleVoiceAI(true);
        setError(null);
        try {
            await onSave({
                version: 2,
                mode: "amplevoice",
                amplevoice: {
                    api_key: amplevoice.api_key.trim(),
                    voice: amplevoice.voice,
                    speed: amplevoice.speed,
                    language: amplevoice.language,
                },
            });
        } catch (err) {
            setError(err instanceof Error ? err.message : "Failed to save configuration");
        } finally {
            setIsSavingAmpleVoiceAI(false);
        }
    };
        ...
        <div className="space-y-6">
            ...
            <Tabs value={mode} onValueChange={(value) => setMode(value as ModelMode)} className="space-y-6">
                <TabsList className="grid w-full grid-cols-3">
                    ...
                    <TabsTrigger value="amplevoice">AmpleVoiceAI</TabsTrigger>
                ...
                <TabsContent value="amplevoice" className="mt-0">
                    <Card>
                        <CardContent className="pt-6">
                            <div className="grid gap-4 sm:grid-cols-2">
                                <div className="space-y-2">
                                    ...
                                    {isCustomVoice ? (
                                        <Input
                                            ...
                                            value={amplevoice.voice}
                                            onChange={(event) => setAmpleVoiceAI({ ...amplevoice, voice: event.target.value })}
                                        />
                                    ) : (
                                        <Select value={amplevoice.voice} onValueChange={(voice) => setAmpleVoiceAI({ ...amplevoice, voice })}>
                                            <SelectTrigger className="w-full">
                                                <SelectValue placeholder="Select voice" />
                                            </SelectTrigger>
                                            <SelectContent>
                                                {defaults.amplevoice.voices.map((voice) => (
                                    ...
                                    {allowCustomVoice && (
                                        <div className="flex items-center space-x-2">
                                            <Checkbox
                                                id="amplevoice-custom-voice"
                                                checked={isCustomVoice}
                                                onCheckedChange={(checked) => {
                                                    const custom = checked as boolean;
                                                    setIsCustomVoice(custom);
                                                    if (!custom) {
                                                        setAmpleVoiceAI({ ...amplevoice, voice: defaults.amplevoice.defaults.voice });
                                                    }
                                                }}
                                            />
                                            <Label htmlFor="amplevoice-custom-voice" className="text-sm font-normal cursor-pointer">
                                                Enter Custom Value
                                            </Label>
                                        </div>
                                    )}
                                ...
                                <div className="space-y-2">
                                    <Label>Speed</Label>
                                    <Select
                                        value={String(amplevoice.speed)}
                                        onValueChange={(speed) => setAmpleVoiceAI({ ...amplevoice, speed: Number(speed) })}
                                    >
                                        <SelectTrigger className="w-full">
                                            <SelectValue placeholder="Select speed" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {defaults.amplevoice.speeds.map((speed) => (
                                ...
                                <div className="space-y-2 sm:col-span-2">
                                    ...
                                    <Select value={amplevoice.language} onValueChange={(language) => setAmpleVoiceAI({ ...amplevoice, language })}>
                                        <SelectTrigger className="w-full">
                                            <SelectValue placeholder="Select language" />
                                        </SelectTrigger>
                                        <SelectContent>
                                            {defaults.amplevoice.languages.map((language) => (
                                ...
                                <div className="space-y-2 sm:col-span-2">
                                    <Label htmlFor="amplevoice-api-key">API Key</Label>
                                    <div className="relative">
                                        <KeyRound className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-muted-foreground" />
                                        <Input
                                            id="amplevoice-api-key"
                                            className="pl-9"
                                            value={amplevoice.api_key}
                                            onChange={(event) => setAmpleVoiceAI({ ...amplevoice, api_key: event.target.value })}
                            ...
                            <Button type="button" className="mt-6 w-full" onClick={saveAmpleVoiceAIConfiguration} disabled={isSavingAmpleVoiceAI}>
                                <Save className="mr-2 h-4 w-4" />
                                {isSavingAmpleVoiceAI ? "Saving..." : submitLabel}
                            </Button>
                        </CardContent>
                    </Card>
                </TabsContent>
